  ...
  do {
    ...
    case S11_CREATE_SESSION_RESPONSE:
      if (itti_s11_create_session_response* m = dynamic_cast<itti_s11_create_session_response*>(msg)) {
        sgw_s11_inst->send_s11_msg(ref(*m));
      }
      break;
...
//------------------------------------------------------------------------------
void sgw_s11::send_s11_msg(itti_s11_create_session_response& i)
{
  send_triggered_message(i.r_endpoint, i.teid, i.gtp_ies, i.gtpc_tx_id);
}
//------------------------------------------------------------------------------
void sgw_s11::send_s11_msg(itti_s11_delete_session_response& i)
{
  send_triggered_message(i.r_endpoint, i.teid, i.gtp_ies, i.gtpc_tx_id);
}
//------------------------------------------------------------------------------
void sgw_s11::send_s11_msg(itti_s11_modify_bearer_response& i)
{
  send_triggered_message(i.r_endpoint, i.teid, i.gtp_ies, i.gtpc_tx_id);
}
//------------------------------------------------------------------------------
void sgw_s11::handle_receive_create_session_request(gtpv2c_msg& msg, const boost::asio::ip::udp::endpoint& remote_endpoint)
{
  ...
  if (!error) {
    ...
    if (RETURNok != ret) {
      Logger::sgwc_s11().error( "Could not send ITTI message %s to task TASK_SGWC_APP", i.get()->get_msg_name());
    }
  }
  // else ignore
}
