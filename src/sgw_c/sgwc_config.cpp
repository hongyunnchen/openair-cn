...
int sgwc_config::load(const string& config_file)
{
  ...
  try
  {
    ...
    const Setting& s1_up_cfg = nw_if_cfg[SGWC_CONFIG_STRING_INTERFACE_S1U_S12_S4_UP];
...
void sgwc_config::display ()
{
  ...
  Logger::sgwc_app().info( "-S1u_S12_S4:");
  Logger::sgwc_app().info( "    iface ............: %s", s1_up.if_name.c_str());
  Logger::sgwc_app().info( "    ipv4.addr ........: %s", inet_ntoa (s1_up.addr4));
  Logger::sgwc_app().info( "    ipv4.mask ........: %s", inet_ntoa (s1_up.network4));
  Logger::sgwc_app().info( "    port .............: %d", s1_up.port);
