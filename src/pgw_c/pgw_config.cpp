...
int pgw_config::load(const string& config_file)
{
  ...
  try
  {
    ...
    const Setting& sgi_cfg = nw_if_cfg[PGW_CONFIG_STRING_INTERFACE_SGI];
    ...
    if (boost::iequals(astring, "yes"))
      ...
      masquerade_SGI = false;
    }

    nw_if_cfg.lookupValue(PGW_CONFIG_STRING_UE_TCP_MSS_CLAMPING, astring);
    if (boost::iequals(astring, "yes"))
      ue_tcp_mss_clamp = true;
    } else {
      ue_tcp_mss_clamp = false;
    }
#endif
...
#if ENABLE_OPENFLOW
    const Setting &ovs_cfg = pgw_cfg[PGW_CONFIG_STRING_OVS_CONFIG];
    if (!(ovs_cfg.lookupValue(PGW_CONFIG_STRING_OVS_BRIDGE_NAME, ovs_config.bridge_name) &&
        ovs_cfg.lookupValue(PGW_CONFIG_STRING_OVS_EGRESS_PORT_NUM, ovs_config.egress_port_num) &&
        ovs_cfg.lookupValue(PGW_CONFIG_STRING_OVS_GTP_PORT_NUM, ovs_config.gtp_port_num) &&
        ovs_cfg.lookupValue(PGW_CONFIG_STRING_OVS_L2_EGRESS_PORT, ovs_config.uplink_mac) &&
        ovs_cfg.lookupValue(PGW_CONFIG_STRING_OVS_UPLINK_MAC, ovs_config.l2_egress_port))) {
      Logger::pgwc_app().error(PGW_CONFIG_STRING_OVS_CONFIG " failed");
    }

    const Setting &arp_list_cfg = ovs_cfg[PGW_CONFIG_STRING_OVS_SGI_ARP_CACHE];
    count = arp_list_cfg.getLength();
    ovs_config.sgi_arp_boot_cache.num_entries = 0;
    for (int i = 0; i < count; i++) {
      const Setting &arp_cfg = arp_list_cfg[i];
      arp_cfg.lookupValue(PGW_CONFIG_STRING_IP, astring);
      IPV4_STR_ADDR_TO_INADDR (astring.c_str(), ovs_config.sgi_arp_boot_cache.ip[i], "BAD IP ADDRESS FORMAT FOR SGi ARP CACHE !");

      arp_cfg.lookupValue(PGW_CONFIG_STRING_MAC, ovs_config.sgi_arp_boot_cache.mac[i]);
      ovs_config.sgi_arp_boot_cache.num_entries++;
    }
#endif
...
void pgw_config::display ()
{
  ...
  Logger::pgwc_app().info( "- SGi:");
  Logger::pgwc_app().info( "    iface ............: %s", sgi.if_name.c_str());
  Logger::sgwc_app().info( "    ipv4.addr ........: %s", inet_ntoa (sgi.addr4));
  if (sgi.network4.s_addr) {
    Logger::sgwc_app().info( "    ipv4.mask ........: %s", inet_ntoa (sgi.network4));
  }
  if (sgi.mtu) {
    Logger::pgwc_app().info( "    MTU ..............: %u", sgi.mtu);
  }
#if ENABLE_LIBGTPNL
  Logger::pgwc_app().info( "    User TCP MSS clamping : %s", ue_tcp_mss_clamp == 0 ? "false" : "true");
  Logger::pgwc_app().info( "    User IP masquerading  : %s", masquerade_SGI == 0 ? "false" : "true");
#endif
...
//------------------------------------------------------------------------------
bool pgw_config::is_apn_handled(const string& apn, const core::pdn_type_t& pdn_type)
{
