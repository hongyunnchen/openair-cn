...
#ifndef SRC_OAI_SGW_COMMON_ITTI_ITTI_HPP_INCLUDED_
#define SRC_OAI_SGW_COMMON_ITTI_ITTI_HPP_INCLUDED_
...
class itti_timer {
public:
  itti_timer(const timer_id_t id, const task_id_t task_id, const uint32_t interval_sec, const uint32_t interval_us) : id(id), task_id(task_id)  {
    time_out = std::chrono::system_clock::now() + std::chrono::seconds(interval_sec) + std::chrono::microseconds(interval_us);
  }
  itti_timer(const timer_id_t id, const task_id_t task_id, const std::chrono::system_clock::time_point time_out) : id(id), task_id(task_id), time_out(time_out)  {
  }
  itti_timer(const itti_timer &t) : id(t.id), task_id(t.task_id) , time_out(t.time_out) {}
...
public:
  itti_task_ctxt(const task_id_t task_id) :
    task_id(task_id), task_state(TASK_STATE_STARTING), msg_queue() ,  m_state(), m_queue(), c_queue() {}
  ~itti_task_ctxt() {}
...
private:
   ...
   */
  std::atomic<unsigned int>                 msg_number;
  timer_id_t                                timer_id;
...
#endif /* SRC_OAI_SGW_COMMON_ITTI_ITTI_HPP_INCLUDED_ */
