...
static itti_timer null_timer(ITTI_INVALID_TIMER_ID, TASK_NONE, 0xFFFFFFFF, 0xFFFFFFFF);
    ...
    if (itti_inst->terminate) return;
    {
      ...
      if (itti_inst->current_timer.time_out > chrono::system_clock::now()) {
        ...
        if ( cv_status::timeout == rc) {
          // signal time-out
          itti_msg_timeout mto(TASK_ITTI_TIMER, itti_inst->current_timer.task_id, itti_inst->current_timer.id);
