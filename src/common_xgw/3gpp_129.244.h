...
#ifndef FILE_3GPP_129_244_SEEN
#define FILE_3GPP_129_244_SEEN
...
public:
  pfcp_msg_bad_length_exception(const uint8_t msg_type, const uint16_t msg_size) throw() {
    phrase = fmt::format("PFCP msg {} Bad Length {} Exception", msg_type, msg_size);
  }
...
public:
  pfcp_msg_illegal_ie_exception(const uint8_t msg_type, const uint16_t ie_type) throw() {
    phrase = fmt::format("PFCP msg {} Illegal {} Exception", msg_type, ie_type);
  }
...
public:
  pfcp_tlv_bad_length_exception(uint16_t ie_type, uint16_t ie_length) throw() : pfcp_tlv_exception(ie_type){
    phrase = fmt::format("PFCP IE TLV {} Bad Length {} Exception", ie_type);
  }
...
//  };
  ...
  typedef struct up_function_features_s {
    uint8_t bucp:1;
    uint8_t ddnd:1;
    uint8_t dlbd:1;
    uint8_t trst:1;
  ...
  //typedef core::fq_csid_t fq_csid_t;
...
#define OUTER_HEADER_CREATION_GTPU_UDP_IPV4 1
#define OUTER_HEADER_CREATION_GTPU_UDP_IPV6 2
#define OUTER_HEADER_CREATION_UDP_IPV4      4
#define OUTER_HEADER_CREATION_UDP_IPV6      8
  ...
  typedef struct cp_function_features_s {
    uint8_t      spare:6;
    uint8_t      ovrl:1;
    uint8_t      load:1;
